...
def estimateleres(img, model, w, h, device):
...
def doubleestimate(img, size1, size2, pix2pixsize, model, net_type, pix2pixmodel, device):
    # Generate the low resolution estimation
    estimate1 = singleestimate(img, size1, model, net_type, device)
    # Resize to the inference size of merge network.
    estimate1 = cv2.resize(estimate1, (pix2pixsize, pix2pixsize), interpolation=cv2.INTER_CUBIC)

    # Generate the high resolution estimation
    estimate2 = singleestimate(img, size2, model, net_type, device)
    # Resize to the inference size of merge network.
...
def singleestimate(img, msize, model, net_type, device):
    # if net_type == 0:
    return estimateleres(img, model, msize, msize, device=device)
    # else:
...
def estimateboost(img, model, model_type, pix2pixmodel, max_res=512, device: str = 'cpu'):
    ...
    whole_estimate = doubleestimate(img, net_receptive_field_size, whole_image_optimal_size, pix2pixsize, model, model_type, pix2pixmodel, device)
    ...
    for patch_ind in range(len(imageandpatchs)):
        ...
        patch_estimation = doubleestimate(patch_rgb, net_receptive_field_size, patch_netsize, pix2pixsize, model, model_type, pix2pixmodel, device)
